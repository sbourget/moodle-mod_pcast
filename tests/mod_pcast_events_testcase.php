...
class mod_pcast_events_testcase extends \advanced_testcase {
    ...
    public function test_comment_created() {
        ...
        $pcast = $this->getDataGenerator()->create_module('pcast', ['course' => $course]);
        ...
        $url = new \moodle_url('/mod/pcast/view.php', ['id' => $pcast->cmid]);
    ...
    public function test_course_module_viewed() {
        ...
        $dbpcast = $DB->get_record('pcast', ['id' => $pcast->id]);
        $context = \context_module::instance($pcast->cmid);
        $mode = 'letter';

        $event = \mod_pcast\event\course_module_viewed::create([
            'objectid' => $dbpcast->id,
            'context' => $context,
            'other' => ['mode' => $mode],
        ]);
    ...
    public function test_course_module_instance_list_viewed() {
        ...
        $event = \mod_pcast\event\course_module_instance_list_viewed::create([
    ...
    public function test_episode_deleted() {
        ...
        $this->assertEquals(CONTEXT_MODULE, $event->contextlevel);
        $this->assertEquals($pcast->cmid, $event->contextinstanceid);
        $expected = [$course->id, "pcast", "delete episode", "view.php?id={$pcast->cmid}&amp;mode={$prevmode}&amp;hook={$hook}", $episode->id, $pcast->cmid, ];
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_episode_approved() {
        ...
        $pcast = $this->getDataGenerator()->create_module('pcast', ['course' => $course, 'requireapproval' => 1]);
        $context = \context_module::instance($pcast->cmid);
